...
def clean_cgm(df):
    ...
    df = df.sort_values(["subject", "visit", "timestamp"])
    df["timestamp"] = df["timestamp"].dt.round("1min")
    gap_minutes = df.groupby(["subject", "visit"])["timestamp"].diff().dt.total_seconds() / 60
    is_phase_shift = (gap_minutes % 15 != 0) & gap_minutes.notna()
    df["segment"] = is_phase_shift.groupby([df["subject"], df["visit"]]).cumsum()
    resampled_df = (df.set_index("timestamp").groupby(["subject", "visit", "segment"]).resample("15min", origin="start").asfreq().reset_index())
    df = resampled_df
